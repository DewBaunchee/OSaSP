...
const int SENDING_SIGNALS[PROCESS_COUNT] = {
    0,       // 0
    SIGUSR1, // 1
    SIGUSR2, // 2
    SIGUSR1, // 3
...
const int RECIPIENTS[PROCESS_COUNT] = {
    0,  // 0
    2,  // 1
    -3, // 2
    6,  // 3
...
void actionLink(int sig, siginfo_t *info, void *text)
{
    usleep(10 * 1000);
    int usrNum = 0;
    if (sig == SIGUSR1)
    ...
    printf("%d %d %d get USR%d %lu\n", selfNumber, getpid(), getppid(), usrNum, getCurrentTime());

    int sendingSignal = SENDING_SIGNALS[selfNumber];
    if (sendingSignal == SIGUSR1)
    {
        usr1Put++;
        usrNum = 1;
    }
    if (sendingSignal == SIGUSR2)
    {
        usr2Put++;
        usrNum = 2;
    }

    if (SENDING_SIGNALS[selfNumber] != 0)
    {
        int processIndex = RECIPIENTS[selfNumber];
        pid_t recipient = (processIndex < 0 ? -1 : 1) * (pids[processIndex < 0 ? -processIndex : processIndex]);
        kill(recipient, sendingSignal);
        printf("%d %d %d put USR%d %lu\n", selfNumber, getpid(), getppid(), usrNum, getCurrentTime());
    }
}
...
void setUsrAction()
{
    ...
    action.sa_sigaction = actionLink;
